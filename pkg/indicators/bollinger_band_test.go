...
91.05	1.55	94.15	87.95	6.20`, "\n")
	...
	window := 20
	sigma := 2.0
	src := indicators.NewClosePriceIndicator(ts)
	sma := indicators.NewSimpleMovingAverage(src, window)
	...
	for i := window - 1; i < len(ts.series.Candles); i++ {
		j := i - (window - 1)
		decimalAlmostEquals(t, decimal.NewFromString(SMAs[j]), sma.Calculate(i), 0.01)
