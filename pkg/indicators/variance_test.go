...
func TestVarianceIndicator(t *testing.T) {
	t.Run("when index is less than 1, returns 0", func(t *testing.T) {
		series := testutils.MockTimeSeries("0", "10")
		stdDev := indicators.NewStandardDeviationIndicator(indicators.NewClosePriceIndicator(series))

		assert.EqualValues(t, "0", stdDev.Calculate(0).String())
	})

	t.Run("returns the standard deviation when index > 2", func(t *testing.T) {
		series := testutils.MockTimeSeriesFl(
			10,
			2,
			38,
			23,
		...
		varInd := indicators.NewVarianceIndicator(indicators.NewClosePriceIndicator(series))
